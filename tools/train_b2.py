...
from models.model_b2 import BaseNet
...
def trainValidateSegmentation(args):
    ...
    config = {
        'purpose': 'B2 symmetric pre-diff DWConv residual gate, validation-only screen',
        'base': 'author repository prepared by user; original MaxPool path',
        'seed': 2333,
        ...
        'test_used': False,
        'model': 'models/model_b2.py',
        'change': 'five shared temporal symmetric pre-diff DWConv residual gates only; no top-down gate; no SSM'
    }
    Path(args.savedir, 'config.json').write_text(json.dumps(config, indent=2))
    shutil.copy2(Path(__file__), Path(args.savedir, 'train_b2.py'))
    shutil.copy2(Path('/content/Mobile-CDNet/models/model_b2.py'), Path(args.savedir, 'model_b2.py'))
    start_epoch = 0
    cur_iter = 0
    max_F1_val = 0
    best_meta = None
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        print('B2 gate scales:', gate_scales)
