...
from models.model_b1r import BaseNet
...
def trainValidateSegmentation(args):
    ...
    config = {
        'purpose': 'B1R strict-init top-down residual selective fusion, validation-only screen',
        ...
        'test_used': False,
        'model': 'models/model_b1r.py',
        'change': 'strict-init B1R: author path initialized through cls before top-down gates; no pre-diff gate; no SSM'
    }
    Path(args.savedir, 'config.json').write_text(json.dumps(config, indent=2))
    shutil.copy2(Path(__file__), Path(args.savedir, 'train_b1.py'))
    shutil.copy2(Path('/content/Mobile-CDNet/models/model_b1r.py'), Path(args.savedir, 'model_b1r.py'))
    start_epoch = 0
    cur_iter = 0
    max_F1_val = 0
    best_meta = None
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        print('B1R gate scales:', gate_scales)
