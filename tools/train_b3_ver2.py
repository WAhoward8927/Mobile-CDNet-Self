...
from models.model_b3_ver2 import BaseNet
...
def trainValidateSegmentation(args):
    ...
    # Validation-only screen: the test split is intentionally not loaded.
    ...
    config = {
        'purpose': 'B3_Ver2 B1R top-down plus c2-only symmetric pre-diff gate, validation-only screen',
        'base': 'author commit e7846b52b316bf2733d70c2c6df213128115e6fe with MaxPool',
        'seed': 2333,
        'dataset_root': args.file_root,
        'input_size': [args.inWidth, args.inHeight],
        'batch_size': args.batch_size,
        'max_steps': args.max_steps,
        'max_epochs_internal': args.max_epochs,
        'validation_log_epochs': [1, args.max_epochs - 1],
        'lr': args.lr,
        'lr_mode': args.lr_mode,
        'step_loss': args.step_loss,
        'loss': 'author BCE_plus_Dice_sum_on_sigmoid_probability',
        'params': int(total_params),
        'test_used': False,
        'model': 'models/model_b3_ver2.py',
        'change': 'B3_Ver2 strict-init: B1R top-down gates at f4/f3/s2 plus one B2 symmetric pre-diff gate at c2; plain abs at c1/c3/c4/c5; no SSM'
    }
    Path(args.savedir, 'config.json').write_text(json.dumps(config, indent=2))
    shutil.copy2(Path(__file__), Path(args.savedir, 'train_b3_ver2.py'))
    shutil.copy2(Path('/content/Mobile-CDNet_B1R/models/model_b3_ver2.py'), Path(args.savedir, 'model_b3_ver2.py'))
    start_epoch = 0
    cur_iter = 0
    max_F1_val = 0
    best_meta = None
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        # Save best validation checkpoint and audit metadata.
        gate_scales = model.swa.gate_scales()
        print('B3_Ver2 gate scales:', gate_scales)
